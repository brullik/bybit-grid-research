...
def _zero_none_contract(e, failures: list[str]) -> None:
    ...
    if e.event_type is EventType.funding:
        if e.order_id is not None or e.level_index is not None or e.side is not None or e.quantity_base != ZERO or e.liquidity_role is not None:
            _fail(failures, "funding stray order/fill fields")
        if e.position_effect is not PositionEffect.none or e.realized_position_pnl_gross_usdt != ZERO or e.trading_fee_usdt != ZERO:
            _fail(failures, "funding stray accounting fields")
    if e.event_type is EventType.termination_trigger:
        if e.order_id is not None or e.level_index is not None or e.side is not None or e.quantity_base != ZERO or e.liquidity_role is not None or e.funding_rate is not None:
...
def audit_simulation_result(result: SimulationResult, mark_price: Decimal | None = None) -> AuditResult:
    ...
    if not _non_bool_int(result.events_rejected_after_termination_count, "events_rejected_after_termination_count"):
    ...
    expected_init = {
        ...
        "buy_orders_below_base_bool": all(init_by_idx.get(i) is not None and init_by_idx[i].side is OrderSide.buy for i, p in enumerate(result.levels) if p < result.config.base_price),
        "sell_orders_above_base_bool": all(init_by_idx.get(i) is not None and init_by_idx[i].side is OrderSide.sell for i, p in enumerate(result.levels) if p > result.config.base_price),
        "base_level_has_no_order_bool": all(o.price != result.config.base_price and o.level_index in expected_indices for o in initial_orders),
        "one_active_order_per_level_bool": len(initial_orders) == len(expected_indices) and len(init_by_idx) == len(initial_orders),
    ...
    if any(type(x) is not str or x == "" for x in order_ids) or len(order_ids) != len(set(order_ids)):
        _fail(failures, "duplicate/empty order_id")
    all_by_id = {o.order_id: o for o in result.all_orders}
    for o in result.all_orders:
        if not (0 <= o.level_index < len(result.levels)) or o.price != result.levels[o.level_index] or not isinstance(o.side, OrderSide) or not isinstance(o.state, OrderState):
        ...
        if o.filled_sequence_id is not None and (not _non_bool_int(o.filled_sequence_id, "filled_sequence_id") or o.activation_sequence_id > o.filled_sequence_id):
    ...
    for e in result.ledger:
        if not _non_bool_int(e.sequence_id, "sequence_id") or not _non_bool_int(e.time_ms, "time_ms"):
        ...
        if e.event_type in (EventType.grid_fill, EventType.termination_fill):
            ...
            if e.event_type is EventType.grid_fill:
                ...
                if o is None or o.state is not OrderState.filled or o.filled_sequence_id != e.sequence_id or o.level_index != e.level_index or o.price != e.price or e.level_index is None or e.price != result.levels[e.level_index] or o.side is not e.side or e.quantity_base != result.config.quantity_per_grid_base or e.liquidity_role is not result.config.grid_fill_liquidity_role:
            ...
            if e.signed_position_after != pos or e.average_entry_after != avg or e.trading_fee_usdt != ZERO or e.funding_pnl_usdt != ZERO or e.realized_position_pnl_gross_usdt != ZERO:
    ...
    for c in result.completed_cycles:
        ...
        gross = oe.quantity_base * ((oe.price if oe.side is OrderSide.sell else ce.price) - (oe.price if oe.side is OrderSide.buy else ce.price))
    ...
    if result.terminated_bool:
        if len(term_triggers) != 1:
            ...
            if result.termination.termination_reason is TerminationReason.lower_boundary and (result.config.lower_termination_price is None or tt.price != result.config.lower_termination_price):
                _fail(failures, "lower termination reason/price mismatch")
            if result.termination.termination_reason is TerminationReason.upper_boundary and (result.config.upper_termination_price is None or tt.price != result.config.upper_termination_price):
                _fail(failures, "upper termination reason/price mismatch")
            if result.termination.termination_reason is TerminationReason.explicit_manual_synthetic and (tt.price <= ZERO):
                _fail(failures, "manual termination trigger invalid")
        if result.signed_position != ZERO or result.average_entry is not None or not result.termination.all_orders_cancelled_bool or not result.termination.position_flat_after_termination_bool:
        ...
        if term_fills:
            ...
            if trigger is not None:
                expected_price = trigger * (Decimal("1") - result.config.termination_slippage_bps / Decimal("10000")) if tf.side is OrderSide.sell else trigger * (Decimal("1") + result.config.termination_slippage_bps / Decimal("10000"))
                if tf.price != expected_price or abs(tf.price - trigger) * tf.quantity_base != result.termination.termination_slippage_cost_usdt:
