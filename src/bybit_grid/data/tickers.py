...
def download_tickers(client: BybitClient, category: str='linear') -> pl.DataFrame:
    ...
    out=metadata_path(client.settings.data_dir, f'tickers_{category}_snapshot_{ts_label()}.parquet'); out.parent.mkdir(parents=True, exist_ok=True); df.write_parquet(out)
    return df
