...
SECRET_PATTERNS = [
    re.compile(r'(X-BAPI-SIGN\s*[:=]\s*)([^,\s}]+)', re.I),
    re.compile(r'(api[_-]?secret\s*[:=]\s*)([^,\s}]+)', re.I),
    re.compile(r'(signature\s*[:=]\s*)([^,\s}]+)', re.I),
    re.compile(r'(secret\s*[:=]\s*)([^,\s}]+)', re.I),
]
SENSITIVE_KEYS = {'bybit_api_key','api_key','apikey','key','bybit_api_secret','api_secret','secret','signature','sign','x-bapi-sign'}
...
def redact(obj: Any) -> Any:
    ...
    if isinstance(obj, str):
        s=obj
        for p in SECRET_PATTERNS: s=p.sub(r'\1***REDACTED***', s)
        return s
    return obj
...
class RedactionFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if record.args: record.args = tuple(redact(a) for a in record.args) if isinstance(record.args, tuple) else redact(record.args)
        return True
