# Sprint 06.4A.3.6 — Production-First Material Tests, Atomic Store Graph and Semantic Seed-Pack Closure

## 0. PM authorization and branch discipline

Sprint 06.3B remains accepted and frozen.

Sprint 06.4A.3.6 is rejected after review of draft PR #67.

Frozen reviewed refs:

```text
REJECTED_PR = 67
REJECTED_BASE_SHA = 70afc0eb4f48bc2e63a85e5e8751a13970f30187
REJECTED_HEAD_SHA = 9dc54abae8fc4d78eeed0bfd96916387c7e07cc4
REJECTED_MERGE_REF_SHA = 0b3c0e5bc1cecbc773dc646bb710b829cfd93388
MAIN_SHA_AT_REVIEW = ae47603ca34030f6a019a9eeb5d99699bbeee570
...
```text
codex/sprint-06-4a-3-6-production-first-semantic-store
```
...
## 0.1 Non-negotiable correction to the rejected PR #67

PR #67 changed only the workflow, manifest, verifier, and four test files. That is insufficient.

The corrected PR **must contain material production diffs** in at least:
...
Change `reader.py`, `writer.py`, `models.py`, `parsing.py`, or other modules only where required by the exact contracts below.

Automatic code-gate failure conditions:

```text
no production market_store files changed
only tests/verifier/docs/workflow changed
any mapped test accepts an arbitrary Exception
any mapped test uses observations.append(type(exc).__name__...)
any mapped test ends with assert observations
any mapped test asserts only type-name strings
any import/preflight test passes object() as evidence
any planner behavior uses empty rows as its fixture
any replay/DuckDB/pack behavior uses a missing store as its only fixture
any lifecycle behavior executes python --version instead of project CLIs
```

The new PR must target `main`, not an intermediate prompt branch.

## 0.2 Exact CI contract

Create or replace `.github/workflows/pull-request.yml` with a Python matrix:
...
For each matrix version run, in this order:
...
all five focused Sprint 06.4A.3.6 modules
...
The workflow must use Python >=3.12. GitHub Actions must be green before PM review. Local claims are not a substitute.

## 0.3 Exact anti-cheating test syntax

Negative tests must use one of these forms:

```python
with pytest.raises(MarketStoreError, match=r"^exact_error$"):
    production_call(...)
```
...
assert result == exact_expected_model
assert snapshot_tree(root) == exact_before_snapshot
```

Forbidden in mapped tests:
...
object() as ValidatedPublicBatchEvidence
empty row tuple for a non-empty planner behavior
missing directory as the fixture for an unrelated behavior
assert isinstance(result, SomeType) as the only assertion
assert result is not None as the only assertion
```

The verifier must reject these AST patterns. It must normalize the test function name to `<TEST_FUNCTION>` before duplicate-body comparison, in addition to normalizing constants.

For each mapped behavior, hard-code and verify:

```text
required production symbols
required fixture/helper calls
required exact error literal or required exact positive-value literals
forbidden fixture literals/calls
```

The manifest cannot select these values; they belong in frozen Python mappings.
...
## 20.1 Exact PR #67 regression tests

Add a dedicated file:

```text
tests/test_sprint_06_4a_3_6_pr67_regressions.py
```

It must prove the verifier rejects source equivalent to every rejected PR #67 pattern:

1. Decimal max test using `Decimal("1.000000000000000000")` and only checking a type name.
2. Planner test calling `partition_validated_rows(..., [])` and accepting either exception or success.
3. Preflight test passing `object()` and asserting only that something happened.
4. Import test passing `object()` and accepting `evidence_type_invalid` for a positive behavior.
5. Audit tamper test auditing only a missing store.
6. Replay test calling only a missing store and accepting any exception.
7. Coverage out-of-window test using only an in-window complete sample.
8. DuckDB success test calling only an invalid/missing store.
9. Pack tamper test calling only missing paths.
10. Full lifecycle test executing only `python --version`.
11. Two identical test bodies differing only by function name.
12. Two identical test bodies differing only by integer/string/path constants.

Each must produce an exact verifier error such as:

```text
forbidden_any_exception_capture
forbidden_observations_pattern
forbidden_object_evidence
forbidden_empty_fixture
forbidden_missing_store_fixture
forbidden_python_version_lifecycle
missing_exact_expected_literal
duplicate_normalized_test_body
```

## 20.2 Required production tamper matrix

The Codex report must include a table with command/test, mutation, expected result, and actual result for:

```text
committed same-PK changed-row preflight
post-version failpoint on new store
post-first-chunk failpoint on new store
post-evidence failpoint on new store
before-receipt failpoint on new store
append failure before receipt on existing store
corrupt Parquet then no-op import
corrupt chunk manifest then no-op import
corrupt evidence archive then no-op import
corrupt evidence reference then no-op import
corrupt receipt then no-op import
orphan chunk
orphan evidence
moved/path-rewritten chunk
nested public review-pack semantic tamper with recomputed ZIP hashes
seed-pack derived report tamper with recomputed manifest hashes
fully fabricated seed pack with internally consistent hashes
```
