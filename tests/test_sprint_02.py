...
def test_universe_filter_excludes_prelaunch_and_non_usdt():
    df = normalize_universe(
        [_inst("AAAUSDT"), _inst("BBBUSDT", pre=True), _inst("CCCUSDC", quote="USDC")],
        [
            {
                "symbol": "AAAUSDT",
            ...
            {"symbol": "BBBUSDT", "turnover24h": "10000000", "lastPrice": "1"},
        ],
    )
...
def test_universe_rank_by_turnover():
    df = normalize_universe(
        [_inst("AUSDT"), _inst("BUSDT")],
        [
            {"symbol": "AUSDT", "turnover24h": "10", "lastPrice": "1"},
            {"symbol": "BUSDT", "turnover24h": "20", "lastPrice": "1"},
        ],
    )
...
def test_fgrid_parser_rejects_ambiguous_markers_and_http_success_fallback(tmp_path):
    meta = {
        "symbol": "XUSDT",
        "lastPrice": 1.0,
        "tickSize": "0.1",
    ...
    payloads = [
        {},
        {"retCode": None, "result": result},
        {"retCode": "0", "result": result},
        {"retCode": True, "result": result},
        {"retCode": 0, "status_code": 400, "result": result},
        {"retCode": 0, "retMsg": None, "result": result},
        {"retCode": 0, "debug_msg": [], "result": result},
        {"retCode": 0, "result": {**result, "debug_msg": 7}},
    ]
    for payload in payloads:
        row = parse_validate_response(meta, payload, status_code=200)
    ...
    semantic_rejection = parse_validate_response(
        meta,
        {"retCode": 0, "result": {**result, "debug_msg": "param error"}},
    )
    ...
    canary = "FGRID_PERSIST_CANARY"
    safe = parse_validate_response(
        meta,
        {
            "retCode": 0,
            "retMsg": canary,
            "result": {**result, "debug_msg": canary},
        },
        status_code=True,
    )
    assert safe["validate_ok"] is True
    assert safe["retMsg"] == "***REDACTED***"
    assert safe["debug_msg"] == "***REDACTED***"
    assert safe["http_status_code"] is None
    path = tmp_path / "safe_constraints.parquet"
    append_constraints(path, [safe])
    persisted = pl.read_parquet(path).to_dicts()
...
def test_download_manifest_respects_launch_time_and_size_cap():
    uni = pl.DataFrame(
        {"symbol": ["AUSDT"], "turnover24h": [10.0], "launchTime": [32503680000000]}
    )
    ...
    with pytest.raises(ValueError):
        build_download_manifest(
            pl.DataFrame(
                {"symbol": ["AUSDT"], "turnover24h": [10.0], "launchTime": [0]}
            ),
            pl.DataFrame(),
            90,
            1,
