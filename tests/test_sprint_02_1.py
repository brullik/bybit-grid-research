...
from bybit_grid.bybit.fgrid_constraints import append_constraints, build_candidate_payloads, candidate_key
...
def test_downloader_skips_blocked_by_default_and_override(tmp_path, monkeypatch):
    ...
    pl.DataFrame({"symbol": ["A"], "trading_feasibility_status": ["blocked_by_min_investment"], "start_ms": [0], "end_ms": [60_000]}).write_parquet("data/processed/download_manifest.parquet")
    import scripts.download_universe_data as d

    d.main.__globals__["__name__"] = "not_main"
    monkeypatch.setattr("sys.argv", ["download", "--manifest", "data/processed/download_manifest.parquet", "--dry-run"])
    d.main()
    ...
    monkeypatch.setattr("sys.argv", ["download", "--manifest", "data/processed/download_manifest.parquet", "--dry-run", "--include-blocked", "--reason", "exploratory_data_only"])
    d.main()
    metrics = json.loads(Path("reports/sprint_02_download_performance_report.json").read_text())
...
def test_candidate_key_includes_stop_loss_and_bounds():
    meta = {"symbol": "A", "range_width_pct": 0.05, "cell_number_requested": 5, "leverage_requested": 1, "init_margin_requested": 100, "stop_loss_mult": 0.95, "min_price": 1, "max_price": 2}
...
def test_append_constraints_dedupes_fresh_and_appended(tmp_path):
    ...
    row = {"symbol": "A", "range_width_pct": 0.05, "cell_number_requested": 5, "leverage_requested": 1, "init_margin_requested": 100.0, "stop_loss_mult": 0.95, "min_price": 1.0, "max_price": 2.0, "feasible_user_5usdt_rule": False}
...
def test_stage_a_candidate_generation_diverse_and_bounded():
    ...
    assert {m["range_width_pct"] for m in metas} == {0.05, 0.1, 0.2}
    assert {m["cell_number_requested"] for m in metas} == {5, 10, 20}
