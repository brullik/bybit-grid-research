...
from scripts.smoke_private_account import SENSITIVE_WALLET_AMOUNT_KEYS, sanitize_private_account_snapshot
...
def test_handle_response_standard_v5_success_retcode_zero_passes():
    ...
    try:
        ...
        assert client._handle_response("/v5/test", _response(payload), "test") == payload
    finally:
        client.close()
...
def test_handle_response_trading_bot_success_status_code_200_passes():
    client = _client()
    try:
        payload = {"status_code": 200, "debug_msg": "", "result": {"ok": True}}
        assert client._handle_response("/bot/test", _response(payload), "test") == payload
    finally:
        client.close()


def test_handle_response_trading_bot_error_status_code_400_raises():
    client = _client()
    try:
        payload = {"status_code": 400, "debug_msg": "bad request", "result": {}}
        with pytest.raises(BybitAPIError) as exc_info:
            client._handle_response("/bot/test", _response(payload, http_status=400), "test")
...
def test_handle_response_trading_bot_error_with_http_200_raises():
    client = _client()
    try:
        payload = {"status_code": 400, "debug_msg": "schema rejected", "result": {}}
        with pytest.raises(BybitAPIError) as exc_info:
            client._handle_response("/bot/test", _response(payload), "test")
        assert exc_info.value.ret_code == 400
        assert exc_info.value.response_data == payload
        assert not _is_retryable(exc_info.value)
    finally:
