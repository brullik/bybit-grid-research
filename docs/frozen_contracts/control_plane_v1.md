...
The activation opens Draft for a combined exact RED. After package installation, the base-owned checker stages the manifest, pytest configuration, and both frozen tests from the immutable head Git object into a separate runner-temp tree and rechecks every staged byte immediately before and after execution. All 52 declared nodes must be collected and each must have exactly one plain failed call with its member sentinel. Any pass, skip, xfail/xpass, deselection, collection/setup/teardown anomaly, duplicate or missing call fails closed. Only after both exact-SHA Draft matrix components pass may the owner emit `ready_for_review` for the unchanged commit; that fresh Ready run repeats every component and is the only state in which the aggregate can succeed and GitHub can merge the activation. Opening directly Ready, any other Ready event, or an untrusted rerun fails closed. Non-probe implementation runs use the complete ordinary base-frozen suite and may proceed only when it is green. Final aggregation rechecks immutable live identity and unresolved reviews. Completion uses the normal separate close commit. Rollback reverts or closes the activation without creating live-execution authority. This path is consumed after one use and creates no v3.
