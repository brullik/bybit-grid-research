...
- `pm-recovery-bundle`: is the one-time, owner-only transition for `p0-recovery-walk-forward-committed-key`, and no generic task resume API. It changes exactly `pm_acceptance/active_task.json` and `pm_acceptance/reactivations/p0-recovery-walk-forward-committed-key.json`, combines only `p0-walk-forward-exclusive-outcome-end` and `p0-committed-key-preflight`, and pins their historical provenance, corrected v1 erratum, exact 12-path union, two sentinels, and 32 + 20 = 52 RED nodes.
...
### One-time recovery activation

The `pm-recovery-bundle` manifest is compact canonical JSON and is absent from base. The base must be the exact v1-erratum-reactivated `p0-walk-forward-exclusive-outcome-end`; first-parent history must contain the pinned historical activations and exactly one suspension of `p0-committed-key-preflight`. The manifest SHA-pins historical active-task, complete frozen-test and contract bytes, the merged v1 erratum commit/manifest, the corrected prior test, the exact 12 allowed/required paths, both failure sentinels, and exactly 52 unique node IDs. Activation is one direct non-merge commit and changes only the task file plus the exact new reactivation manifest. Any replay, task substitution, changed-then-reverted provenance, file-mode drift, missing/extra path, noncanonical value, or generic recovery attempt fails closed.
