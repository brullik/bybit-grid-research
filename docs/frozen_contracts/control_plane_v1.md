...
Activation requires canonical repository and first-parent `main` history, exactly one #157 suspension without completion or replay, and exactly one merged valid v1 erratum for #156 as its current predecessor. The owner and live event sender must both be `brullik`; base/head repositories, main ref, immutable base/head SHAs, event identity, exact single label, exact two-path diff, absent-on-base manifest, and one direct non-merge activation commit are revalidated. Canonical JSON rejects BOMs, duplicate/unknown/missing keys, floats, Boolean integers, unsafe or noncanonical paths and IDs, duplicate members or nodes, substitutions, stale order, change-revert, divergence, and unpinned input.

The activation remains Draft for a combined exact RED: all 52 declared nodes must be collected and each must have exactly one plain failed call with its member sentinel. Any pass, skip, xfail/xpass, deselection, collection/setup/teardown anomaly, duplicate or missing call fails closed. Non-probe implementation runs use the complete ordinary base-frozen suite and may proceed only when it is green. Final aggregation rechecks immutable live identity and unresolved reviews. Completion uses the normal separate close commit. Rollback reverts or closes the activation without creating live-execution authority. This path is consumed after one use and creates no v3.
