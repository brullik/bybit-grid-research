...
Issue #210 adds one exceptional, fail-closed activation mode and no generic recovery mechanism. Bundle `p0-recovery-walk-forward-committed-key` contains only #156 `p0-walk-forward-exclusive-outcome-end` and #157 `p0-committed-key-preflight`. Its immutable identities are the historical activation commits `1305abb1517944e2cc9790e5546ca52ae66f592e` and `3b826f2a6a3b02897047a30de8e920e2f5b72431`, their pinned active-task, frozen-test and frozen-contract SHA-256 values, exact 32 and 20 node sets, member sentinels, and the union of their exact eight and four required paths.

Activation requires canonical repository and first-parent `main` history, exactly one #157 suspension without completion or replay, and exactly one merged valid v1 erratum for #156 as its current predecessor. The owner, live event sender, and current `github.triggering_actor` must all be `brullik`, and `github.run_attempt` must be a positive canonical integer; base/head repositories, main ref, immutable base/head SHAs, event identity, exact single label, exact two-path diff, absent-on-base manifest, and one direct non-merge activation commit are revalidated. Canonical JSON rejects BOMs, duplicate/unknown/missing keys, floats, Boolean integers, unsafe or noncanonical paths and IDs, duplicate members or nodes, substitutions, stale order, change-revert, divergence, and unpinned input.

The activation opens Draft for a combined exact RED. After dependency installation, the base-owned checker reconstructs the complete regular-file head tree in runner temp from the immutable Git object. It requires the archive path set to equal the commit tree, verifies every file against its Git blob ID, preserves executable identity as staged read-only modes, and rechecks the exact path, byte, and mode set immediately before and after execution. The RED subprocess uses isolated no-site startup, does not inherit Python or pytest path controls, and explicitly adds only the staged repository roots and physical interpreter dependency directories; editable-install path files are never processed. Every imported `bybit_grid` and `scripts` module is also required to resolve inside the staged tree. All 52 declared nodes must be collected and each must have exactly one plain failed call with its member sentinel. Any import-root leak, staged-tree mutation, pass, skip, xfail/xpass, deselection, collection/setup/teardown anomaly, duplicate or missing call fails closed. Only after both exact-SHA Draft matrix components pass may the owner emit `ready_for_review` for the unchanged commit; that fresh Ready run repeats every component and is the only state in which the aggregate can succeed and GitHub can merge the activation. Opening directly Ready, any other Ready event, or an untrusted rerun fails closed. Non-probe implementation runs use the complete ordinary base-frozen suite and may proceed only when it is green. Final aggregation rechecks immutable live identity and unresolved reviews. Completion uses the normal separate close commit. Rollback reverts or closes the activation without creating live-execution authority. This path is consumed after one use and creates no v3.
