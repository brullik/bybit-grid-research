...
try:
    ...
    print(f"skipped: {e}")
    write_sprint_report(settings.data_dir, {"account info summary": f"skipped: {e}"})
    raise SystemExit(0)
...
out.write_text(
    redacted_json_dump({"account_info": info, "wallet_balance": wallet}), encoding="utf-8"
)
write_sprint_report(
    settings.data_dir, {"account info summary": "saved redacted account_info_redacted.json"}
)
